...
void full_decrypt(char* cryptFile, char* keyfile){
	clock_t start = clock();
	clock_t end;
	FILE *fp;
	FILE *fp_out,*fp_key;
...
int totalEncrypt(const char* inputFile, const char* keyfile, const char* outputFile){
	clock_t start = clock();
	clock_t end;
	int x;
	int k = 0;
...
//		totalEncrypt("zImage", "keyfile2.txt","crypt2.jpg");
		...
		full_decrypt("photo.jpg","keyfile1.txt");
}
